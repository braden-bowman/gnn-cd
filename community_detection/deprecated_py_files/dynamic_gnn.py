...
from .gnn_community_detection import (nx_to_pyg, extract_embeddings,
...
def generate_dynamic_graphs(n_time_steps=5, n_nodes=100, n_communities=5, 
    ...
    G_init = nx.stochastic_block_model(
        [n_nodes // n_communities] * n_communities,
        p_in=0.3 * np.eye(n_communities) + 0.05 * (1 - np.eye(n_communities)),
        seed=seed
    )
    
    # Add community labels
    for i in range(n_nodes):
        comm = i // (n_nodes // n_communities)
        G_init.nodes[i]['community'] = comm
...
def visualize_dynamic_communities(graphs, community_attr='community', figsize=(15, 10),
                                layout='fixed', title="Communities Over Time"):
    """
